...
MatchManager = (function() {
    return {
        addMatch: function(seedDoc, matchDoc) {
            ...
            if (toRemove) {
                logger.debug("Already a match between " + seedDoc._id + " and " + matchDoc._id);
            } else {
        ...
        removeMatch: function(seedDoc, matchDoc) {
            ...
            var toRemove = DocMatches.find({userID: user._id,
                                            ...
                                            matchDocID: matchDoc._id});
            if (toRemove) {
                logger.trace("Existing matches: " + JSON.stringify(toRemove.fetch()));
                ...
                logger.debug("No existing matches");
                return false;
            }
        }
    }
